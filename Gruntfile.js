...
module.exports = function (grunt) {
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new hilink name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/hilink.png')) {
            fs.renameSync(__dirname + '/admin/hilink.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/hilink.html')) {
            fs.renameSync(__dirname + '/widgets/hilink.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/hilink/js/hilink.js')) {
            fs.renameSync(__dirname + '/widgets/hilink/js/hilink.js', __dirname + '/widgets/hilink/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/hilink')) {
            fs.renameSync(__dirname + '/widgets/hilink',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Hilink/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Hilink'
                        },
                        {
                            match: /bondrogeen/g,
                            replacement: author
